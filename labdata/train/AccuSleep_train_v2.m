...
for i = 1:nFiles
    ...
    if isfield(data.a, 'EEG1')
        data.a.EEG = data.a.EEG1;
    elseif isfield(data.a, 'EEG2')
        data.a.EEG = data.a.EEG2;
    elseif isfield(data.a, 'EEG3')
        data.a.EEG = data.a.EEG3;
    elseif isfield(data.a, 'EEG4')
        data.a.EEG = data.a.EEG4;
    else 
    end
    ...
    for j = (pad+1):(length(processedEMG)-pad)
        ...
        if data.c.labels(j) > 0 && data.c.labels(j) < 4
            ...
            fName = [imageLocation,filesep,num2str(data.c.labels(j)),...
                filesep,'rec',num2str(i),'t',num2str(j-pad,'%05.f')];
            fName = strcat(fName,"*",csname(end-2),"*",csname(end-1),'.png');
            % write to file
...
fileInfo = cellfun(@(x) split(x, '*'), imds.Files, 'UniformOutput', false);

% Extract cohort and animal labels from fileInfo
cohortLabels = cellfun(@(x) x{end-1}, fileInfo, 'UniformOutput', false);
animalLabels = cellfun(@(x) x{end-0}, fileInfo, 'UniformOutput', false);

% Determine the unique cohort labels and their counts in the entire dataset
uniqueCohorts = unique(cohortLabels);
cohortCounts = countcats(categorical(cohortLabels));

% Normalize counts to obtain ratios
totalSamples = sum(cohortCounts);
cohortRatios = cohortCounts / totalSamples;

% Specify the desired ratio for each cohort in the training set
desiredTrainRatio = cohortRatios; % Use normalized ratios

% Loop over each cohort to split data into training and validation sets
trainIndices = [];
validationIndices = [];

for i = 1:length(uniqueCohorts)
    % extracting cohort indices 
    cohortIndices = find(ismember(cohortLabels, uniqueCohorts{i}));
    
    % Randomly permute the indices for each cohort to ensure randomness
    cohortIndices = cohortIndices(randperm(length(cohortIndices)));
    
    % Determine the number of samples for training based on the desired ratio
    numTrainSamples = round(desiredTrainRatio(i) * cohortCounts(i));
    
    % Randomly select samples for training
    trainIndices = [trainIndices; cohortIndices(1:numTrainSamples)];
    
    % Assign the rest to the validation set
    validationIndices = [validationIndices; cohortIndices(numTrainSamples+1:end)];
end

% Shuffle the combined training and validation indices to ensure randomness
combinedIndices = randperm(length(trainIndices) + length(validationIndices));
trainIndices = combinedIndices(1:length(trainIndices));
validationIndices = combinedIndices(length(trainIndices)+1:end);


% Create training and validation sets using the determined indices
...
save('trained_network.mat', 'net', 'trainInfo');
